...
Vue 3 控制台、宏录制、八个脚本槽位和五段板载任务说明，请参阅：
...
- 固件内置 4 个可替换示例宏：天埠罗巢穴刷武器、杏棱巢穴刷钱、武器分解和连接手柄；第 5～8 槽预留给自定义宏。
- 共管理 8 个宏槽位。网页保存的脚本写入 Flash，同槽位下会优先于 C++ 内置脚本运行；删除网页覆盖即可恢复内置版本。
...
- 支持 8 个可配置的 GPIO 启动触发器和 1 个停止触发器，保存后无需浏览器也能启动指定宏。
...
管理 8 个宏槽位、载入或新建宏，并将最多 5 个宏按顺序组合为板载宏循环。
...
设备页可以保存 8 条 GPIO 启动规则（每条可绑定一个宏槽位，或启动已保存的宏循环），以及 1 条停止规则。固件只允许安全白名单中的引脚，并使用带校验和的事务协议保存配置。配置成功后，即使没有打开网页或拔掉 USB-UART，按下已配置的 GPIO 仍可启动对应宏或宏循环；但拔线不会终止已经在运行的宏。
...
| `START` / `MACRO_START slot` | 从第 1 步启动当前宏，或直接启动指定的 `0～7` 槽位 |
| `STOP` | 停止流程并发送一份完全中立的手柄报告 |
| `STATUS` | 返回当前阶段、步骤、循环次数和时间信息 |
| `PING` | 返回 `PONG` |
| `R buttons dpad lx ly rx ry` | 发送一份临时的完整 HID 报告 |
| `MACRO_GET` / `MACRO_LIST` | 读取当前宏，或读取 8 个槽位与当前槽位信息 |
...
- `firmware/src/MacroLibrary.cpp` — 8 槽网页脚本 Flash 存储
