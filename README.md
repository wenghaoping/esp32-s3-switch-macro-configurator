...
> [!IMPORTANT]
> **一块 ESP32-S3：原生 USB 负责模拟 Switch 有线手柄，按需 Wi-Fi 热点负责网页配置。**
...
- 提供嵌入式 Vue 3 单页控制台：首页、控制、脚本库、编辑器、录制器和设备/GPIO 页面可直接由 ESP32 热点访问。
...
2. 正常启动后长按 BOOT 键 3 秒。
3. 电脑或手机连接开放热点 `ESP32-S3-Switch`，访问 <http://192.168.9.1>。

热点会在重启、再次长按 BOOT 或网页中主动关闭后停止。USB-UART 仍用于烧录和日志。

板载 RGB 灯会提示热点状态：按住 BOOT 时蓝色快闪；热点已开启但尚未连接设备时蓝色双闪；
有设备连接热点后青色常亮。热点关闭后恢复正常状态灯颜色：空闲绿色、单宏青色、任务紫色。

首页、控制、宏设置、录制和设备设置使用 Hash 路由，可从顶部导航进入；也可以使用 `?mock=1` 打开模拟设备模式，方便不接开发板时查看界面。

## 使用方法

1. 正常启动后长按 **BOOT** 3 秒，连接 `ESP32-S3-Switch` 热点。
2. 打开 <http://192.168.9.1>，等待状态显示为 **已连接 · 待命**。
