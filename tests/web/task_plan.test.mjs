...
const slots = Array.from({ length: 8 }, (_, slot) => ({ slot, occupied: slot < 3, name: `宏${slot + 1}` }));
...
test("allows duplicate slots, rejects empty slots and more than five entries", () => {
  ...
  assert.match(validateTaskPlan({ ...plan, entries: [{ slot: 7, repeatCount: 1, gapMs: 0 }] }, slots)[0], /空/);
